...
void ABlasterPlayerController::InitChat()
{
	...
	if (Chat == nullptr)
	{
		Chat = CreateWidget<UChat>(this, ChatWidget);
		Chat->AddToViewport();
		Chat->SetVisibility(ESlateVisibility::Collapsed);
	}
}

void ABlasterPlayerController::ShowChat()
{
	if (Chat)
	{
		bChatOpen = !bChatOpen;
		UE_LOG(LogTemp, Warning, TEXT("The boolean value is %s"), (bChatOpen ? TEXT("true") : TEXT("false")));
		if (bChatOpen)
		{
			Chat->ChatSetup();
			if (!Chat->ChatInput->OnTextCommitted.IsBound() && Chat->ChatInput)
			{
				Chat->ChatInput->OnTextCommitted.AddDynamic(this, &ABlasterPlayerController::OnChatCommitted);
			}
		...
		{
			Chat->ChatTearDown();
			if (Chat->ChatInput->OnTextCommitted.IsBound()) {
				Chat->ChatInput->OnTextCommitted.RemoveDynamic(this, &ABlasterPlayerController::OnChatCommitted);
			}
		}
...
void ABlasterPlayerController::BroadcastChatCommitted_Implementation(const FText& Text, const FString& PlayerName)
{
	GameMode = GameMode == nullptr ? Cast<ABlasterGameMode>(UGameplayStatics::GetGameMode(this)) : GameMode;
	if (GameMode)
	{
		
		GameMode->SendChatMsg(Text, PlayerName);
	}
...
void ABlasterPlayerController::ClientChatCommitted_Implementation(const FText& Text, const FString& PlayerName)
{
	if (Chat)
	{
		Chat->OnTextCommitted(Text, PlayerName);
	}
}
...
void ABlasterPlayerController::OnChatCommitted(const FText& Text, ETextCommit::Type CommitMethod)
{
	...
	if (CommitMethod == ETextCommit::OnEnter) {
		...
		BroadcastChatCommitted(Text, PlayerName); //TODO: Add a delay so that some absolute cucumber can't spam the chat.
	}
...
void ABlasterPlayerController::SetupInputComponent()
{
	...
	InputComponent->BindAction("Chat", IE_Pressed, this, &ABlasterPlayerController::ShowChat);
...
void ABlasterPlayerController::AcknowledgePossession(APawn* InPawn) {
...
void ABlasterPlayerController::UpdateDeathMessage(FString KilledBy)
{
...
void ABlasterPlayerController::SetHUDWeaponIcon()
{
	BlasterHUD = BlasterHUD == nullptr ? Cast<ABlasterHUD>(GetHUD()) : BlasterHUD;
	...
	if (bHUDValid && Weapon && Weapon->WeaponIcon)
	{
		BlasterHUD->CharacterOverlay->WeaponTypeImage->SetBrushFromTexture(Weapon->WeaponIcon, true);
	}
	else {
		bIntializeWeaponIcon = true;
	}
}


void ABlasterPlayerController::CleanWeaponIcon()
{
	BlasterHUD = BlasterHUD == nullptr ? Cast<ABlasterHUD>(GetHUD()) : BlasterHUD;
	bool bHUDValid = BlasterHUD &&
		BlasterHUD->CharacterOverlay &&
	...
	if (bHUDValid)
	{
		BlasterHUD->CharacterOverlay->WeaponTypeImage->SetBrushFromTexture(nullptr, true);
	}
}
	...
	if (CharacterOverlay == nullptr)
	{
		if (BlasterHUD && BlasterHUD->CharacterOverlay)
		{
			...
			if (CharacterOverlay)
			{
				...
				if (bIntializeWeaponIcon)	SetHUDWeaponIcon();
